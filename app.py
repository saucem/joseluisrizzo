...
mail = Mail(app)
folder = os.path.join('static','images','upload')
...
def home():
  ...
  if request.method == "POST":
    searchTerm = request.form['criteria']
    if searchTerm.lower() != "todos":
      sqlSentence = "SELECT * FROM vehicle WHERE title LIKE %s OR brand LIKE %s OR model LIKE %s"
      cur.execute(sqlSentence, ('%' + searchTerm + '%','%' + searchTerm + '%','%' + searchTerm + '%'))
    else:
      sqlSentence = "SELECT * FROM vehicle ORDER BY title ASC"
      cur.execute(sqlSentence)
    showCarousel = False
  else:
    searchTerm = "Todos"
  ...
  if len(importedVehicles) == 0:
    flash('No hemos encontrado vehículos que coincidan con "' + str(searchTerm) + '" en la base de datos.', category='not-found-error')

  return render_template('index.html', categories = importedCategories, vehicles = importedVehicles, criteria = searchTerm, showCarousel = showCarousel)
...
def vsearch():
  ...
  return render_template('search_result.html', categories = importedCategories, vehicles = importedVehicles, criteria = searchTerm)

@app.route('/search_by_domain', methods = ['GET', 'POST'])
def dsearch():

  cur = dbconnection.connection.cursor()

  if request.method == "POST":
    searchDomain = request.form['domain']
    sqlSentence = "SELECT * FROM vehicle WHERE domain = '" + str(searchDomain) + "'"
    cur.execute(sqlSentence)
    importedVehicles = cur.fetchall()
    ...
    if len(importedVehicles) == 0:
      ...
      flash('No hemos encontrado vehículos con el dominio "' + str(searchDomain), category='error')
      return render_template('vehicles_abm.html', categories = importedCategories, vehicles = importedVehicles)
    else:
      vehicleID = importedVehicles[0][0]
      return redirect('../vhc_edit/' + str(vehicleID))
